...
public class EnnemiDistance extends Ennemi {
    ...
    public Projectile attaquerJoueur(Joueur joueur){
        ...
        if(this instanceof EnnemiBoss) { tailleProjectile=24;}
        else{ tailleProjectile=8;}
        ...
        if(Math.abs(joueur.getCentreJoueurX()-getCentreActeurX()) < Math.abs(joueur.getCentreJoueurY()-getCentreActeurY())) {
            if (joueur.getCentreJoueurY() < getCentreActeurY() )
                p = new Projectile(this.getX()+4, this.getY()+4, "UP", "Ennemi", this.getClass().getSimpleName(), tailleProjectile);
            else
                p = new Projectile(this.getX()+4, this.getY()+4, "DOWN", "Ennemi", this.getClass().getSimpleName(), tailleProjectile);
        }
        else{
            if(joueur.getCentreJoueurX() < getCentreActeurX())
                p = new Projectile(this.getX()+4, this.getY()+4, "LEFT", "Ennemi", this.getClass().getSimpleName(), tailleProjectile);
            else
                p = new Projectile(this.getX()+4, this.getY()+4, "RIGHT", "Ennemi", this.getClass().getSimpleName(), tailleProjectile);
        }
        return  p;
    }
